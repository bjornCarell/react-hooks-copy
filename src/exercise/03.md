...
When updating a component with a child component, React (1) renders the parent
from start to end, (2) render the child from start to end, (3) Runs the parent
useEffect cleanups, (4) Runs the child useEffect updates, (5) runs
the parent useEffect updates.

When unmounting the Child, e.g. hide it in the DOM, React (1) renders the
parent form start to end, (2) runs the child useEffect cleanups, (3) runs
the parent useEffect clean up, and (4) the parent useEffect updates.
## Background
