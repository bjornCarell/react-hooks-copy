...
Elaborate on your learnings here in `src/exercise/05.md` useRef gives us
accessto the current object, the current object is mutable. Each time we want
access to something that we want to be able to change, without triggering
are-render of our component, we can make use of useRef and it's current object.
