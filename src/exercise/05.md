...
useRef gives us access to the current object, the current object is mutable. 
So each time we want access to something that we want to be able to change,
without triggering a re-render of our component, we can make use of useRef 
and it's current object. 
## Background
